...
object Run extends App {
  ...
  val data_base = "file:///home/ali/workspace/SPARK/Spark_velib_analyser/data"
  val avscfile = data_base+"/velib/stations_velib.avsc"
  val csvfile  = data_base+"/velib/stations_velib.csv"
  val avrofile = data_base+"/velib/stations_velib.avro"
  val files_semaine = data_base+"/velib/semaine/*.json"
  val files_weekend = data_base+"/velib/weekend/*.json"
  val outputpath = data_base+"/output/"
  ...
  val data_pre_aggregated_s = data_semaine
      .groupBy("fields.number").agg(sum("fields.bike_stands").alias("pre_sum_bike_stands"), sum("fields.available_bikes").alias("pre_sum_available_bikes"))
